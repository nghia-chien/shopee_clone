...
import { useSellerThreads } from '../../hooks/useChat';
...
export function SellerChatPage() {
  ...
  const { data: threadsData, isLoading } = useSellerThreads();
  ...
  const getLastMessagePreview = (thread: ChatThread) => {
    if (thread.messages && thread.messages.length > 0) {
      ...
      if (lastMsg.sender_type === 'SYSTEM') {
        return t('chat.system_message_preview');
      }
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between mb-1">
                        <h3 className="font-semibold text-gray-900 truncate">
                          {thread.user?.name || t('chat.customer')}
                        </h3>
          ...
          {selectedThread ? (
            <>
              <div className="p-4 border-b border-gray-200 bg-gray-50">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 bg-blue-500 rounded-full flex items-center justify-center text-white font-semibold">
                    {selectedThread.user?.name?.charAt(0).toUpperCase() || 'U'}
                  </div>
                  <div>
                    <h3 className="font-semibold text-gray-900">
                      {selectedThread.user?.name || t('chat.customer')}
                    </h3>
              ...
              <div className="flex-1 min-h-0">
                <ChatBox threadId={selectedThread.id} isSeller={true} />
              </div>
            </>
          ) : (
